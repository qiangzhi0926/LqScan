 ...
 */
- (void)addUI{
    self.maskView = [[ZFMaskView alloc] initWithFrame:CGRectMake(0,  0, SCREEN_WIDTH, 780)];
    NSLog(@"width is %f, height is %f",SCREEN_WIDTH,SCREEN_HEIGHT);
   [self.view addSubview:self.maskView];
    ...
    self.scanStateLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH - 20, flashlightHint_height)];
    self.scanStateLabel.text = @"正在扫描";
    self.scanStateLabel.textAlignment = NSTextAlignmentCenter;
    self.scanStateLabel.textColor = ZFWhite;
    ...
    //竖屏
        ...
        self.scanStateLabel.center = CGPointMake(SCREEN_WIDTH/2, 167);
    }
}
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection{
    if (metadataObjects.count > 0) {
        ...
        NSLog(@"scan or_str is :%@",metadataObject.stringValue);
...
//        UIAlertController * view = [UIAlertController alertControllerWithTitle:@"扫描已完成" message:@"是否继续" preferredStyle:UIAlertControllerStyleActionSheet];
...
- (void)viewWillTransitionToSize:(CGSize)size withTransitionCoordinator:(id <UIViewControllerTransitionCoordinator>)coordinator{
    ...
    //竖屏(转之前是竖屏，转之后是横屏)
        ...
        self.scanStateLabel.center = CGPointMake(SCREEN_WIDTH/2, SCREEN_HEIGHT - 150);
